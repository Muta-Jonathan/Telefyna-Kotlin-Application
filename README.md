...
Telefyna monitors the `telefynaAudit` directory for specific files to trigger immediate system actions:

- **init.txt**: Forces a hot reload of the schedule from `config.json` without restarting the app.
- **restart.txt**: Immediately restarts the Telefyna application.
- **reboot.txt**: Immediately reboots the device (requires root/system permissions).

These files are deleted automatically after being processed.
