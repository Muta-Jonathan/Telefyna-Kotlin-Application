...
import org.avventomedia.app.telefyna.ForegroundTracker
import org.avventomedia.app.telefyna.R

@OptIn(UnstableApi::class)
class RestartReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        // If app has a visible activity, it's safe to bring it to front; otherwise, post a notification
        if (ForegroundTracker.isInForeground) {
            ...
            try {
                ...
                // If starting the Activity fails (shouldn't when foreground), show a notification fallback
    ...
    private fun showBringToFrontNotification(context: Context) {
        ...
        val builder = NotificationCompat.Builder(context, Telefyna.CHANNEL_ID)
    ...
    companion object {
        ...
        fun scheduleRestart(context: Context, delayMs: Long) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                am.setExactAndAllowWhileIdle(AlarmManager.RTC, System.currentTimeMillis() + delayMs, pending)
            } else {
